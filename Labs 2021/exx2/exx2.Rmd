...
set.seed(my.seed)
class.0 <- mvrnorm(45, mu = c(25, 36), 
                   Sigma = matrix(c(5, 0, 0, 11), 2, 2, 
                                  byrow = T))

set.seed(my.seed + 1)
class.1 <- mvrnorm(55, mu = c(26, 21), 
                   Sigma = matrix(c(4, 0, 0, 16), 2, 2, 
                                  byrow = T))
...
y.grid <- rules.mv(as.matrix(cbind(x1.grid, x2.grid)),
                   c(25, 36), c(26, 21), 
                   matrix(c(5, 0, 0, 11), 2, 2, byrow = T),
                   matrix(c(4, 0, 0, 16), 2, 2, byrow = T))
...
Точность на обучающей выборке высокая и близка к идеалу. Сделаем прогноз классов Y на тестовую выборку и оценим точность модели.
...
Acc
```
Байесовский метод разделяет классы на обучающей выборке, не ошибаясь. Точность идеальна.
