...
$$mpg=f(weight + displacement + acceleration + cylinders)$$
Линейная модель: 

$$mpg=β_0+β_1⋅weihgt +β_2 displacement +β_3 acceleration+ cylinders$$
...
fit.lm.1_1 <- lm(mpg ~ weight + displacement + acceleration + cylinders, subset = inTrain)

# подгонка линейной модели на обучающей выборке
fit.lm.1_1 <- lm(mpg ~ weight + displacement + acceleration + cylinders, 
               subset = inTrain)
# прогноз на тестовую
...
$$mpg = β_0 + β_1weight + β_2 displacement + β_3 acceleration + β_4weight^2 + β_5 displacement^2 + β_6 acceleration^2 + β_7 cylinders + β_8 sylinders^2$$
...
# подгонка модели на обучающей выборке
fit.lm.2_1 <- lm(mpg ~ poly(weight, 2) + poly(displacement, 2) + poly(acceleration, 2) + cylinders, subset = inTrain)
...
$$mpg=β_0+β_1weight+β_1weight + β_2 displacement + β_3 acceleration+ β_5 displacement^2 + 

+β_6 acceleration^2+ + β_4weight^2  β_4weight^2 + β_5 displacement^2 + β_6 acceleration^2+β_7⋅weight^3+ β_8 displacement^3 + β_9 acceleration^3 + β_10 sylinders + β_11 sylinders^2 + β_12 sylinders^3$$
...
# подгонка модели на обучающей выборке
fit.lm.3_1 <- lm(mpg ~ poly(weight, 3)  + poly(displacement, 3) + poly(acceleration, 3) + cylinders, 
               subset = inTrain)
...
# подгонка линейной модели на обучающей выборке
fit.glm_1 <- glm(mpg ~ weight + displacement + acceleration + cylinders, data = DF.auto)
...
for (i in 1:5) {
  # оценка модели
  fit.glm_1 <- glm(mpg ~ poly(weight, i)  + poly(displacement, i) + poly(acceleration, i) + cylinders, data = DF.auto)
  # расчёт ошибки
...
for (i in 1:5) {
  ...
  fit.glm_1 <- glm(mpg ~ poly(weight, i) + poly(displacement, i) + poly(acceleration, i) + cylinders, data = DF.auto)
  # расчёт ошибки
  cv.err.k.fold5_1[i] <- cv.glm(DF.auto, fit.glm_1, K = 3)$delta[1]
}
...
$$mpg=f(weight + displacement + acceleration)$$
Линейная модель: 

$$mpg=β_0+β_1⋅weihgt +β_2 displacement +β_3 acceleration$$
...
# подгонка модели на обучающей выборке
fit.lm.1 <- lm(mpg ~ weight + displacement + acceleration, subset = inTrain)

# подгонка линейной модели на обучающей выборке
fit.lm.1 <- lm(mpg ~ weight + displacement + acceleration, 
               subset = inTrain)
# прогноз на тестовую
...
$$mpg = β_0 + β_1weight + β_2 displacement + β_3 acceleration + β_4weight^2 + β_5 displacement^2 + β_6 acceleration^2$$
...
# подгонка модели на обучающей выборке
fit.lm.2 <- lm(mpg ~ poly(weight, 2) + poly(displacement, 2) + poly(acceleration, 2), subset = inTrain)
...
+β_6 acceleration^2+ + β_4weight^2  β_4weight^2 + β_5 displacement^2 + β_6 acceleration^2+β_7⋅weight^3+ β_8 displacement^3 + β_9 acceleration^3$$
...
# подгонка модели на обучающей выборке
fit.lm.3 <- lm(mpg ~ poly(weight, 3)  + poly(displacement, 3) + poly(acceleration, 3), 
               subset = inTrain)
...
for (i in 1:5) {
  # оценка модели
  fit.glm <- glm(mpg ~ poly(weight, i)  + poly(displacement, i) + poly(acceleration, i), data = DF.auto)
  # расчёт ошибки
...
boot.fn.2 <- function(data, index){
  coef(lm(mpg ~ poly(weight, 2) + poly(displacement, 2) +  poly(acceleration, 2), data = data, subset = index))
}
